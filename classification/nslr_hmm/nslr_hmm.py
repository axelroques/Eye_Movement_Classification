...
def gaze_observation_model():
    ...
    params = {
        ...
        SMOOTH_PURSUIT: [[0.8175021916433242, 0.3047120126632254], [
            [0.13334607025750783, 0.0], [0.0, 2.5328705587328173]]]
    }
...
def viterbi(initial_probs, transition_probs, emissions):
    ...
    for emission in emissions:
        ...
        probs = safelog(emission) + \
            trans_probs[most_likely_states, np.arange(n_states)]
...
def dataset_features(data, **nslrargs):
    segments = ((nslr.fit_gaze(ts, xs, **nslrargs), outliers)
                for ts, xs, outliers in data)
...
def reestimate_observations_baum_welch(sessions,
    ...
    # Should probably have a nicer stopping criterion
    for iteration in range(n_iterations):
        ...
        for features in sessions:
            ...
            probs, forward, backward = forward_backward(
                transition_probs, liks, initial_probs)
            all_state_probs.extend(probs)
            all_transition_probs.append(transition_estimates(
                liks, transition_probs, forward, backward))
        ...
        if plot_process:
            ...
            for cls in np.unique(winner):
                my = winner == cls
                plt.plot(all_observations[my, 0], all_observations[my,
                                                                   1], '.', alpha=0.1, color=CLASS_COLORS[cls+1])
        dists = {}
...
def segment_features(segments, outliers=None):
    ...
    for segment in segments:
        if np.any(outliers[segment.i[0]:segment.i[1]]):
            continue
...
def classify_segments(segments,
    ...
    observation_likelihoods = (observation_model.liks(f)
                               for f in segment_features(segments))

    path = viterbi(initial_probabilities, transition_model,
...
class NSLR_HMM:
    ...
    def __init__(self, t, x, y, **kwargs):
        ...
        self.fit_params = {
            k: kwargs[k]
            for k in ('structural_error', 'optimize_noise', 'split_likelihood')
            if k in kwargs
        }
    ...
    def process(self):
        ...
        eye = np.vstack((self.x, self.y)).T
        self.segmentation = nslr.fit_gaze(self.t, eye, **self.fit_params)
        self.seg_classes = classify_segments(self.segmentation.segments)
        self.sample_classes = np.zeros(len(self.t))

        for c, s in zip(self.seg_classes, self.segmentation.segments):
            start = s.i[0]
            end = s.i[1]
            self.sample_classes[start:end] = c
    ...
    def plot(self):
        ...
        Fixations are shown in blue, saccades in red, PSO in orange
        and pursuits in green.
        """

        _, axes = plt.subplots(2, 1, figsize=(15, 8))

        colors = ['royalblue', 'crimson', 'orange', 'lightgreen']
        timestamps = self.segmentation.t
        x = [val[0] for val in self.segmentation.x]
        y = [val[1] for val in self.segmentation.x]
        ...
        # Legend
        fix = mpatches.Patch(color=colors[0], label='Fixations')
        sacc = mpatches.Patch(color=colors[1], label='Saccades')
        pso = mpatches.Patch(color=colors[2], label='PSO')
        pursuit = mpatches.Patch(color=colors[3], label='Pursuits')
        plt.legend(handles=[fix, sacc, pso, pursuit], fontsize=15)
